...
<section class="content">
    <div class="row">
        <div class="col-lg-12" style="margin-bottom: 10px">  
            <a class="btn btn-info" href="{{ route('admin.vaccine.create') }}">
                <i class="fa-solid fa-plus"></i>
                <span style="margin-left: 5px">Thêm mới</span>
            </a>
            @can('viewAny', App\User::class)
            <a class="btn btn-primary" href="{{ route('admin.vaccine.trash') }}">
        ...
        <div class="col-lg-12">
            <div class="box box-primary">
                <div class="box-header">
                    <h3 class="box-title">Danh sách<span class="label label-info" style="margin-left: 8px">{{ $vaccines->total() }} kết quả</span></h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
                    <table id="example1" class="table table-bordered table-striped">
                        ...
                        <tbody>
                            ...
                            <tr class="item-{{ $vaccine->id }}">
                                <td class="text-center">{{ $key + 1 }}</td>
                                <td class="">{{ $vaccine->name }}</td>
                                <td class="text-center">
                                    <span class="label label-{{ ($vaccine->is_active == 1) ? 'success' : 'danger' }}">{{ ($vaccine->is_active == 1) ? 'Hiển thị' : 'Ẩn' }}</span>
                                </td>
                                <td class="text-center">

                                    {{-- <button type="button" class="btn btn-primary btn-detail" data-toggle="modal" data-target="" title="Chi tiết" data-id="{{$vaccine->id}}">
                                        <i class="fa-solid fa-eye"></i>
                                    </button> --}}

                                    <a href="{{ route('admin.vaccine.edit', ['id'=> $vaccine->id]) }}" class="btn btn-warning" title="Sửa">
                                        <i class="fa-solid fa-pencil"></i>
                                    </a>

                                    <a href="javascript:void(0)" class="btn btn-danger" onclick="destroyModel('vaccine', '{{ $vaccine->id }}' )" title="Xóa">
                                        <i class="fa fa-trash"></i>
                                    </a>
...
<script src="/myAssets/js/vaccine/index.js"></script>
@endsection
