                ...
                <form class="">
                    ...
                    <div class="box-body">
                        <div class="form-group d-flex" id="form-name">
                            ...
                            <div>
                                <input id="name" name="name" type="text" class="form-control" placeholder="Tên vaccine" value="{{ $vaccine->name }}">
                            </div>
                        </div>
                        <div class="form-group" id="form-disease_id">
                            ...
                            <div class=""">
                                <select class="form-control select2" multiple="multiple" data-placeholder="Chọn dịch bệnh" style="width: 100%;" name="disease_id" id="disease_id">
                                    ...
                                    <option value="{{ $disease->id }}" {{ $vaccine->belongsToManyDiseases->contains("id", $disease->id) ? 'selected' : '' }} >{{ $disease->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group" id="form-producer_id">
                            ...
                            <div class="">
                                <select class="form-control select2" multiple="multiple" data-placeholder="Chọn nhà sản xuất" style="width: 100%;" name="producer_id" id="producer_id">
                                    ...
                                    <option value="{{ $producer->id }}" {{ $vaccine->belongsToManyProducers->contains("id", $producer->id) ? 'selected' : '' }} >{{ $producer->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="form-group d-flex" style="flex-direction: column" id="form-description">
                            ...
                            <div style="width: 100%;">
                                <textarea id="description" name="description" class="form-group">{{ $vaccine->description }}</textarea>
                            </div>
                        </div>
                        <div class="checkbox form-group" id="form-is_active">         
                            <label>
                                <input type="checkbox" name="is_active" value="1" {{ $vaccine->is_active == 1 ? 'checked' : '' }} >Trạng thái hiển thị 
                            </label>
                        </div>
                    </div>
                    <!-- /.box-body -->
                    ...
                    <div class="box-footer">
                        <a href="javascript:void(0)" class="btn btn-primary edit-vaccine" data-id="{{ $vaccine->id}}">Sửa</a>
